...
/**
 * Compute the per-item diff and summary counts between incoming items and
 * existing items (keyed by tree path). `mode='full'` produces `removed`
 * entries for items present in the DB but absent from the upload.
 */
export function computeDiff(opts: {
  ...
  if (opts.mode === 'full') {
    for (const [key, o] of opts.oldByKey) {
      if (!newByKey.has(key)) {
